...
def au_ingestion(fireball, stations, fieldsums_folder):
    ...
    for station, fieldsum_file in stations.items():
        stations_data[station] = preprocessing.ingestStationData(f'{DATA_DIR}{fieldsums_folder}/{fieldsum_file}')
...
def au_preprocessing(fireball, avg_window, std_window, FILE_NAME, target_time):
    ...
    with open(f'./pickle/{fireball}.pkl', 'rb') as station_data_file:
        ...
        visualizations.plot_intensities(detrended_datasets, f'./plots/{FILE_NAME}')
